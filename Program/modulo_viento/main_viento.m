function [m_CargasE,Data]= main_viento(V,Exp,Cat,Edif,z,L,B,SepCerch,theta,ModeloCerch,Epsilon,PosBloq)
%
...
%% Estados
% Los Estados pueden ser, como máximo, 8 en total , en función de los casos de carga
% que comprende el Reglamento y la pendiente de la cubierta. Inicialmente
% se consideran que los 8 existen y a medida que las condiciones se
% verifiquen, se van eliminando aquellos que no son válidos para el presente
% caso. Los estados son:
...
% Nota01: Para cubierta abovedada se utilizan Est 1 al 4 y 7 a 8.
% Nota02: Para el caso de cubiertas aisladas se utilizan los estados 1 y 2.
% No se considera el esfuerzo por fricción, por no considerarse en el
% modelo. No se consideran los 3 estados distintos para cubiertas a dos
% aguas, por no considerarse en el modelo.
...
switch Edif
    ...
    case 'Cerrado'
        GCpi = [+0.18 -0.18];
    otherwise
        error('Error en la selección del tipo de edificación. Opciones: Cerrado, PCerrad, Aislado.')
end

switch Edif
    case {'PCerrad','Cerrado'}
        % Cp: figura 3
        [m_MatPresNet_vpc, m_MatPresNet_vnc, m_EtiqMatPresNet_vpc, m_EtiqMatPresNet_vnc, Estados] = MatrizPresionesNetas(z, L, B, theta, qz, GCpi, Estados, ModeloCerch,Flecha);
    ...
    case 'Aislado'
        if ModeloCerch == 4
            error('ERROR: El reglamento 102 no contempla cubiertas aisladas con cubierta abovedada.')
        end
        [m_Cpn,m_EtiqMatPresNet] = MatrizPresionesNetas_CubAisl(theta,ModeloCerch,Epsilon,PosBloq);
        p_vnc = [qz(end).*G.*m_Cpn(1), 1, 0, 0, 0, 0, 0, 9;
                 qz(end).*G.*m_Cpn(1), 1, 0, 0, 0, 0, 0, 13;
                 qz(end).*G.*m_Cpn(2), 0, 1, 0, 0, 0, 0, 11;
                 qz(end).*G.*m_Cpn(2), 0, 1, 0, 0, 0, 0, 13;];
        if Cenefa
            % Al existir cenefa, se consideran las fuerzas de presión y
            % succión en las mismas, para ambos casos de viento.
            warning('Se considera una cenefa de Xm de altura')
            AltCenefaIzq = 1.02;
            AltCenefaDer = 1.50;
            CpBar = 1.3;
            CpSot = 0.6;
            p_vnc = [p_vnc;
                    qz(1).*G.*CpBar, 1, 0, 0, 0, 0, 0, 1;
                   -qz(1).*G.*CpSot, 1, 0, 0, 0, 0, 0, 5;
                    qz(1).*G.*CpBar, 0, 1, 0, 0, 0, 0, 1;
                   -qz(1).*G.*CpSot, 0, 1, 0, 0, 0, 0, 5];
        end
end
%% Impresión por pantalla de los resultados
if 0 %;PrintRes
        PrintData.m_MatPresNet_vpc = m_MatPresNet_vpc;
        PrintData.m_MatPresNet_vnc = m_MatPresNet_vnc;
        PrintData.m_EtiqMatPresNet_vpc = m_EtiqMatPresNet_vpc;
        PrintData.m_EtiqMatPresNet_vnc = m_EtiqMatPresNet_vnc;
        PrintData.p_vpc = p_vpc;
        PrintData.p_vnc = p_vnc;
        ImpresionResultados(Edif,PrintData)
end
...
% En caso de existir cenefa se modifican las longitudes de las cargas de
% las columnas.
if Cenefa && strcmp(Edif,'Aislado')
    L_Tr14(1) = AltCenefaIzq;
    L_Tr14(7) = AltCenefaDer;
end
...
for iEst = 1:4
    if Estados(iEst)
        ...
        for j_iTr = 1:size(iTr,2)
            aux_vect = [aux_vect p_vnc(p_vnc(:,end)==iTr(iEst,j_iTr) & p_vnc(:,iEst+1)==1,1)*SepCerch];
            if isempty(p_vnc(p_vnc(:,end)==iTr(iEst,j_iTr),1))
                aux_vect = [aux_vect 0];
            end
        end
        if any(ModeloCerch == [1 2 3 5 6])
