function Datos = p_AreaEfect(Datos)
%
...
if strcmp(PerfilTipo,'G') || strcmp(PerfilTipo,'C')
    efec_1 = (be_E1 - c) < 1e-4;
else
    efec_1 = 1;
end
...
%% Elemento 3 - Ala
% En el caso de un análisis a flexión, el ala traccionada corresponde a un
% Elemento 5, el cual es completamente efectivo.
switch Analisis
...
switch Analisis
    ...
    case {'FlexEjMay'}
        ...
        lambda_E4 = sqrt(f1/Fcr);
        rho_E4 = (1-.22/lambda_E4)/lambda_E4;
        bee = (lambda_E4 <= 0.673) * a + (lambda_E4 > 0.673) * rho_E4*a;
        if A/B <= 4
            be1 = bee/(3+Psi);
            be2 = (Psi>0.236) * (bee/2) + (Psi<=.236) * (bee-be1);
            if (be1+be2) > (a/2)
                be_E4 = a; %alma totalmente efectiva
            else
                warning('*** Si el alma no es totalmente efectiva, hay que revisar el cálculo de la parte efectiva')
                keyboard
                be_E4 = be1 + be2;
            end
        else
...
efec_4 = abs(be_E4 - a) < 1e-4;
%% Área efectiva
% El área efectiva se determinará sumando todas las áreas efectivas
% determinadas para cada elemento.
switch PerfilTipo
    case {'C','G'}
        Aefect = t* (2*be_E1 + 4*be_E2 + 2*be_E3 + be_E4);
    case 'U'
        Aefect = t* (2*be_E2 + 2*be_E3 + be_E4);
end

% Se guarda el % de efectividad del perfil
if (Aefect-Area)<1e-4 && (efec_1+efec_3+efec_4==3)
    % disp('Perfil totalmente efectivo')
    Efectiv = 1;
elseif abs(Aefect-Area)<1e-4 && (efec_1+efec_3+efec_4~=3)
    warning('*** Hay un problema en el cálculo de la efectividad de la sección')
    keyboard
else
    Efectiv = Aefect/Area;
    if strcmp(Datos.Analisis,'FlexEjMay')
        ...
        Datos.efec_3 = efec_3;
        Datos.efec_4 = efec_4;
        Datos.be_E1 = be_E1; Datos.be_E2 = be_E2; 
        Datos.be_E3 = be_E3; Datos.be_E4 = be_E4;
        Datos.be1 = be1; Datos.be2 = be2;
