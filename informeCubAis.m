function informeCubAis(Prob_data,Design) 
% RellenosParaInforme
global model
...
fprintf(['Tipo de edificio: ' Prob_data.Cargas.Carac '\n'])
fprintf('Relación de bloqueo ζ = %1.2f \n', Prob_data.Cargas.Epsilon)
if Prob_data.Cargas.PosBloqueo==0
   AuxStr = 'alero mas bajo.';
else
   AuxStr = 'alero mas alto.';
end
if model == 5
   fprintf(['Posición de bloqueo: ' AuxStr ' \n'])
end
...
%
% Su determinación para cubiertas aisladas está definida por el Anexo I del
% reglamento el cual establece la siguiente expresión:
%
% *p = qh · G · Cpn [N/m2]*
%
...
fprintf('||   Caso de   |     Coeficientes  || \n');
fprintf('||       Carga |     Globales  || \n');
fprintf('||   Máx. p/todo ζ    |       %1.2f     || \n', Prob_data.Cargas.m_Cpn(1));
fprintf('||   Mín. p/ζ=%1.2f   |       %1.2f    || \n',[Prob_data.Cargas.Epsilon Prob_data.Cargas.m_Cpn(2)]);
%%
% Tabla 2: Coeficientes Cpn.

fprintf('||   Caso de   |     Presiones  || \n');
fprintf('||       Carga |     Globales [N/m²] || \n');
fprintf('||   Máx. p/todo ζ    |       %1.2f     || \n', Prob_data.Cargas.p_v(1,1));
fprintf('||   Mín. p/ζ=%1.2f   |       %1.2f    || \n',[Prob_data.Cargas.Epsilon Prob_data.Cargas.p_v(3,1)]);
%%
...
AePred = Prob_data.Mat.Hormigon.Geom.nm*(Prob_data.Mat.Hormigon.Geom.De/10)^2*pi()/4*(420*10/1.75*2/Prob_data.Mat.Hormigon.Geom.b/100/Prob_data.Mat.Hormigon.Geom.Se/100);
fprintf('Armadura de corte: Ae = %1.0f Ø%1.0fc/%1.0fcm\n',[Prob_data.Mat.Hormigon.Geom.nm Prob_data.Mat.Hormigon.Geom.De Prob_data.Mat.Hormigon.Geom.Se*100])
